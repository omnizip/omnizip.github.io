...
**Blog is model-driven (SSOT):** a post is one markdown file in `src/content/blog/` named `{date}-{slug}.md` — the filename is canonical and the frontmatter (`title`, `date`, `excerpt` required; optional `category`, `stats` list) must agree with it. `src/models/blog.js` loads all files via `import.meta.glob('?raw')`, parses frontmatter with js-yaml's CORE schema (dates stay strings — quote any excerpt containing a colon), validates loudly at build/dev time (including filename-date == frontmatter-date), and sorts newest-first. `BlogPost.vue` is the single dynamic renderer (`/blog/:slug`, markdown-it with `html: false`); `BlogIndex.vue` lists from the same model. Adding a post = adding one dated `.md` file, nothing else.
