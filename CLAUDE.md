...
**Routes** (`src/router/index.js`): `/` (Home), `/omnizip`, `/cabriolet`, `/rust` (omnizip-rs product page), `/blog` (post index) and `/blog/omnizip-rs-announcement`. Blog post metadata lives in `src/views/blog/posts.js` — adding a post means a view, a route, and a registry entry. Product docs for the Ruby gems live on external sites (linked via config), not as routes here. Note: in production, `www.omnizip.org/<repo>/` paths are served by the org's other GitHub Pages sites (e.g. `/omnizip/` is the Ruby gem's just-the-docs site), so this SPA's deep routes rely on `public/404.html` redirecting to `/?path=<route>` (restored in `index.html` before the router loads). Scroll behavior restores saved position, smooth-scrolls to hashes, otherwise resets to top.
